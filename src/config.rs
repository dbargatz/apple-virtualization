use crate::bindings::{
    self, VZVirtualMachineConfiguration_VZVirtualMachineConfigurationValidation,
};
use bindings::{INSObject, IVZVirtualMachineConfiguration};

use crate::{bootloader::VZBootLoader, foundation::NSError};

pub struct VZVirtualMachineConfiguration(bindings::VZVirtualMachineConfiguration);
impl VZVirtualMachineConfiguration {
    pub fn new() -> Self {
        let inner = bindings::VZVirtualMachineConfiguration::alloc();
        let inner = unsafe {
            let ptr = inner.init();
            bindings::VZVirtualMachineConfiguration(ptr)
        };
        Self(inner)
    }

    pub fn into_inner(self) -> bindings::VZVirtualMachineConfiguration {
        self.0
    }
